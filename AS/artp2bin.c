...
void printUsage(void) { printf("usage: artp2bin.exe inputcodefile.p outputromfile.bin sharefile.h\n"); }
bool buildRom(FILE* from, FILE* to);
void editShareFile(void);
...
//		printf("arg %d is %s\n", i, argv[i]);
	...
	{
		printf("\nartp2bin.exe: generating %s from %s", romFileName, codeFileName);
